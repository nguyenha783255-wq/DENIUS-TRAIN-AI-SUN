...
app = FastAPI(title='Denius Train AI', version='1.0.0')

@app.on_event('startup')
def startup():
    db.init_db()
    # Web service is intentionally read/API only when a separate Render worker is deployed.

@app.get('/health')
def health():
    return {'ok': True, 'rounds': db.count_rounds()}
...
def status():
    ...
    return {
        ...
        'model': ({'version': active['version'], 'name': active['model_name'], 'trained_rows': active['trained_rows'],
                   'trained_at': active['trained_at'], 'metrics': active['metrics']} if active else None),
...
</style></head><body><div class="wrap"><div class="hero"><h1>🧠 DENIUS TRAIN AI</h1><div class="muted">Lưu lịch sử bền vững · huấn luyện theo thời gian · walk-forward validation · champion/challenger</div></div>
